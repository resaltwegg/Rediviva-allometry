...
allometries$lcl.i <- exp(confint(m2)[1:nspecies,1])
allometries$ucl.i <- exp(confint(m2)[1:nspecies,2])
allometries$lcl.s <- confint(m2)[(nspecies +1):(nspecies*2),1]
allometries$ucl.s <- confint(m2)[(nspecies +1):(nspecies*2),2]
```

The models above fitted separate allometric curves to each species, i.e. $ln(leg) = ln(a) + b \times ln(body)$. The slopes estimate $b$ for each species and exponentiating the intercepts estimates $a$ for each species. Table \ref{tab:alloparms} below gives the parameter estimates with confidence intervals and Figure \ref{fig:staticallometry} plots the fitted relationships.
...
allometries <- allometries[,c(7:10, 1:6)]
knitr::kable(allometries, digits = 2, caption = 'Summary of morphological data and allometric parameters for 25 species of \\textit{Rediviva} bees. LegLength: mean total length of foreleg length [mm]; LL.SD: standard deviation of leg length; BodySize: inter-tegula distance as a measure of body size; BS.SD: standard deviation of body size; intercepts: parameter a of the allometric relationship; slopes: parameter b of the allometric relationships; lcl.i, ucl.i: lower and upper 95% confidence limits of parameter a; lcl.s, ucl.s: lower and upper 95% confidence limits of parameter b.')
```
...
```{r staticallometry, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Allometric relationship of leg length against body size in \\textit{Rediviva} species. The solid circles indicate the species means."), echo=F}
...
```{r}
# calculate mean body length per species:
mean.body <- aggregate(beemorph$log.body, by = list(beemorph$Taxa), FUN=mean)  
colnames(mean.body) <- c("Taxa", "mean.body")

# add mean body length back onto data frame:
beemorph <- merge(beemorph, mean.body, by.x = "Taxa", by.y = "Taxa", all.x=T)  
# difference in body length within species:
beemorph$diff.body <- beemorph$log.body - beemorph$mean.body  
```

The evolutionary allometry describes how a trait scales with body size across species. From Figure \ref{fig:staticallometry} it is clear that *R. gigas* is an outlier in that respect. It is the largest bee in this data set but has average leg length. As it also is ecologically quite different from the other species, we exclude it from the main analysis but for completeness report results also with this species included.
...
According to AIC, Model `m5` is clearly better. The variances for the intercepts and slops estimated by model `m5` are of similar magnitude (intercepts: `r round(as.numeric(VarCorr(m5)[1]),3)`; slopes: `r round(as.numeric(VarCorr(m5)[2]),3)`) adding another piece of evidence suggesting that there is considerable variation in the allometric slope parameter among species (Figure \ref{fig:evolutionaryallometry}).
...
```{r evolutionaryallometry, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Allometric relationship of leg length against body size in \\textit{Rediviva} species. The black line shows the fitted evolutionary allometric relationship."), echo=F}
...
Re-running the analyses in the previous section without *R. neliana*, we find that the AIC for Model `m4` is `r round(AIC(m4),2)` and for Model `m5` `r round(AIC(m5),2)`. So, according to AIC, Model `m5` is still clearly better. While the variance for the slopes decreased compared to the analysis that included *R. neliana*, the variances for the intercepts and slops estimated by model `m5` are still of similar magnitude (intercepts: `r round(as.numeric(VarCorr(m5)[1]),3)`; slopes: `r round(as.numeric(VarCorr(m5)[2]),3)`) suggesting that there is considerable variation in the allometric slope parameter among species that is not entirely explained by *R. neliana* being different from the other species (Figure \ref{fig:evolutionaryallometry_withoutNeliana}.

```{r evolutionaryallometry_withoutNeliana, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Allometric relationship of leg length against body size in \\textit{Rediviva} species, excluding \\textit{R. neliana}. The black line shows the fitted evolutionary allometric relationship."), echo=F}
...
knitr::kable(tab, digits = 2, caption = 'Difference (delta) between the evolutionary and static allometric slopes estimated by three different models across \\textit{Rediviva} species. lcl and ucl are the lower and upper limits of the 95% confidence interval.')
```
...
We re-fitted models `m6` to `m8` including *R. gigas* and the table below gives the estimated difference between the evolutionary and static allometric slopes (delta $=\beta_b - \beta_w$) with confidence intervals:
...
````{r table2.gigas, echo=FALSE, message=FALSE, warnings=FALSE, results='asis'}
tab <- data.frame(Model = c('m6', 'm7', 'm8'), delta = c(coefficients(m6)[3], intervals(m7)$fixed[3,2], intervals(m8)$fixed[3,2]), lcl = c(confint(m6)[3,1], intervals(m7)$fixed[3,1], intervals(m8)$fixed[3,1]), ucl = c(confint(m6)[3,2], intervals(m7)$fixed[3,3], intervals(m8)$fixed[3,3]))

knitr::kable(tab, digits = 2,caption = 'Difference (delta) between the evolutionary and static allometric slopes estimated by three different models across \\textit{Rediviva} species including \\textit{R. gigas}. lcl and ucl are the lower and upper limits of the 95% confidence interval.')
```

Including *R. gigas*, the analysis still suggested that the evultionary allometric is steeper than the static allometric slope but the difference was smaller and the confidence interval overlapped zero for Models `m7` and `m8`. In conclusion, including the outlier species *R. gigas* affects the results presented earlier, albeit not the general pattern. Figure \ref{fig:withgigas} below shows, however, that the fitted evolutionary allometric line is a poor description of the general relationship between log body size and log leg length across species. 
...
Our data set contains multiple individuals per species and we therefore need to add tips to the pylogenetic tree:
...
Next, we use phylogenetic linear regression models. We first add phylogenetic model structures to the errors using model `m3`, which is the simplest multi-level model without random effects:
...
allometries.n$lcl.i <- exp(confint(m2)[1:nspecies,1])
allometries.n$ucl.i <- exp(confint(m2)[1:nspecies,2])
allometries.n$lcl.s <- confint(m2)[(nspecies +1):(nspecies*2),1]
allometries.n$ucl.s <- confint(m2)[(nspecies +1):(nspecies*2),2]
...
allometries.n <- allometries.n[,c(7:10, 1:6)]
knitr::kable(allometries.n, digits = 2, caption = 'Summary of morphological data and allometric parameters for 10 populations of \\textit{R. neliana} bees. LegLength: mean total length of foreleg length [mm]; LL.SD: standard deviation of leg length; BodySize: inter-tegula distance as a measure of body size; BS.SD: standard deviation of body size; intercepts: parameter a of the allometric relationship; slopes: parameter b of the allometric relationships; lcl.i, ucl.i: lower and upper 95% confidence limits of parameter a; lcl.s, ucl.s: lower and upper 95% confidence limits of parameter b.')
...
```{r ea_neliana, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Allometric relationship of leg length against body size in \\textit{R. neliana} populations The black line shows the fitted evolutionary allometric relationship."), echo=F}
...
knitr::kable(tab, digits = 2, caption = 'Difference (delta) between the evolutionary and static allometric slopes estimated by three different models across \\textit{R. neliana} populations. lcl and ucl are the lower and upper limits of the 95% confidence interval.')
```
...
Figure \ref{fig:LegLengthSpurLength} shows leg length as a function of spur length (using a weighted mean across all plant species each bee species is known to visit). *R. gigas* is included in this plot. Leg length tracks spur length with most species having legs at least as long as the spur length of the flowers they visit.
...
```{r LegLengthSpurLength, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Leg length of \\textit{Rediviva} individuals of different species against spur length of their host plant flowers. The solid black line is the 1:1 relationship. Colours indicate different species using the same colour scheme as in Figure \\ref{fig:staticallometry}.)"), echo=F}
...
m11.OUr <- phylolm(intercepts ~ spur_length, phy = beetree, model = "OUrandomRoot", 
                   data = allometries.r)
m11.OUf <- phylolm(intercepts ~ spur_length, phy = beetree, model = "OUfixedRoot", 
                   data = allometries.r)
...
knitr::kable(tab, digits = 2, caption = 'Relationship between the allometric intercept and spur length of the host flower for 18 species of \\textit{Rediviva} bees. Beta is the estimated slope of this relationship and the other columns give the lower and upper 95% confidence limits.')
```
...
knitr::kable(tab, digits = 2, caption = 'Relationship between the allometric slope and spur length of the host flower for 18 species of \\textit{Rediviva} bees. Beta is the estimated slope of this relationship and the other columns give the lower and upper 95% confidence limits.')
```
...
For these reasons, we would like to better understand how spur length of the host plant(s) varies within and across populations, and how the within-population variability in spur length compares to variability in the leg length of the bees. We have such detailed data for two bee species. One of them is *R. neliana*, which we have seen exhibits considerable allometric variation among populations. We will examine this variation in relation to spur length in the next section.

Here, we look at the other species for which we have population-specific data for both bees and their host plants, *R. pallidula*. We do not have the raw data for spur length but we have mean spurlength, standard deviation and sample sizes for six populations. First, we need to do some data preparation:
...
knitr::kable(tab, digits = 3, caption = 'Ratio of the variances in bee leg length and plant spur length')
...
Figure \ref{fig:LegLengthSpurLength_neliana} shows leg length as a function of spur length. Leg length tracks spur length reasonably well across populations.
...
```{r LegLengthSpurLength_neliana, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Leg length of \\textit{R. neliana} individuals from different populations against spur length of their host plant flowers. The solid black line is the 1:1 relationship. Colours indicate different populations using the same colour scheme as in Figure \\ref{fig:ea_neliana}.)"), echo=F}
...
op <- par(mfrow=c(1,3))
plot(intercepts ~ population_spur_length, data=allometries.n, col=col_vector[population_code], axes=F, xlab="Spur length [mm]", ylab="Allometric intercept", pch=19)
axis(1)
axis(2, las=1)
abline(m11.n)

plot(slopes ~ population_spur_length, data=allometries.n, col=col_vector[population_code], axes=F, xlab="Spur length [mm]", ylab="Allometric slope", pch=19)
axis(1)
axis(2, las=1)
abline(m12.n)

plot(slopes ~ population_spur_length, data=allometries.n,type='n',axes=F, ann=F)
legend(6,1, legend = levels(allometries.n$population_code), col=col_vector, bty="n", pch=19, cex=0.9, text.font=3)
dev.off()
```

```{r allometry_spurlength_neliana, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Relationship of allometric parameters (intercept a in the left-hand panel and slope b in the right-hand panel; see Equation \\ref{eq:allometry}) to spur length of the host plant for \\textit{Rediviva} bee species"), echo=F}
...
dev.off()
```
```{r spurvariation_neliana, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Spur length of plant populations that are associated with ten different populations of \\textit{R. neliana} bees. The bees used different \\textit{Diascia} species across populations. In population SP, they used four different \\textit{Diascia} species: \\textit{D. cordata, D. austromontana, D. megathura} and \\textit{D. integerrima}. In population ST, they used two different plant species: \\textit{D. tugelensis} and \\textit{D. vigilid}. The foreleg length of \\textit{R. neliana} in the different populations are shown in orange. Solid and open circles show the population mean of the plant spur length and bee leg length, respectively. The vertical lines extend one standard deviation above and below the mean."), echo=F}
knitr::include_graphics("spurvariation_neliana.pdf")
```

Figure \ref{fig:spurvariation_neliana} suggests that spur length varied relatively little across populations of *D. rigescens* while one of the *D. capsularis* populations had clearly shorter spurs. Most of the variation in spur length across populations was however due to bees visiting different plant species. Where the bees visited different plant species, in populations SP and ST, these plants had flowers with similar spur length and variation in spur length within these populations was not larger than in populations where bees visited a single species. Across populations, bee leg length appeared to be less variable than spur length.
