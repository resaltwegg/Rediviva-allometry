...
```{r, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Allometric relationship of leg length against body size in *Rediviva* species, excluding *R. neliana*. The black line shows the fitted evolutionary allometric relationship."), echo=F}
...
# m5
plot(log.leg ~ log.body, data=beemorph, col=col_vector[SPECIES.no], axes=F, 
     xlim=c(min(beemorph$log.body),max(beemorph$log.body)+0.4), xlab = "", ylab = "")
mtext(expression ('log' * ' body size (ITD)'), side=1, line=2.4)
axis(1, at=seq(0.8, 1.5, 0.2))
axis(2,las=1)
...
```{r withgigas, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Allometric relationship of leg length against body size in *Rediviva* species, including *R. gigas*. The black line shows the fitted evolutionary allometric relationship."), echo=F}
...
A phylogeny published by Kahnt et al [-@Kahnt] included most of the species in our sample. Here is the phylogenetic tree:
