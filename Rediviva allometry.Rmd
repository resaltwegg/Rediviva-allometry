...
This document is an appendix to the paper 'Allometric relationships shape foreleg evolution of long-legged oil bees (Melittidae: Rediviva)' by Annalie Melin, Res Altwegg, John C. Manning and Jonathan F. Colville, detailing the statistical analyses presented there. The data are published on UCT's data portal (DOI: 10.25375/uct.13297640).
...
plot(b ~ spur_length, data=allometries.g,type='n',axes=F, ann=F)
legend(0,1.1, legend = levels(as.factor(rownames(allometries.g))), col=col_vector, bty="n", pch=19, cex=0.55, text.font=3)
...
plot(blupintercepts ~ log(population_spur_length+0.5), data=allometries.n,type='n',axes=F, ann=F)
legend(1.8,2.3, legend = levels(as.factor(rownames(allometries.n))), col=col_vector[allometries.n$population_code], bty="n", pch=19, cex=0.55, text.font=3)
