...
```{r staticallometry, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Allometric relationship of leg length against body size in \\textit{Rediviva} species"), echo=F}
...
where $\sigma_{0j}$ is the estimated variance of the random effects $\mu_{0j}$ and $\sigma_{rj}$ is the estimated variance of the random effects $\mu_{rj}$. We excluded *R. gigas* from this analysis because this species was an outlier with a large influence on the estimated evolutionary allometric relationship.
...
plot(mean.spur.lngth ~ as.numeric(pops), data=dat, ylim=c(3,14), xlim=c(0.8,11), axes=F, col=col_vector.short[Host.plant.species], type='p', pch=19, ylab="Spur or leg length [mm]", xlab="Population")
axis(1, at=c(1:8), labels=dat$pops)
axis(2,las=1)
...
points(seq(1.1,8.1,by=1), dat$p.mean, pch=1, col="darkorange")
for (i in 1:dim(dat)[1]) segments(i+0.1,dat$p.mean[i]+dat$p.sd[i], y1=dat$p.mean[i]-dat$p.sd[i], col="darkorange")

legend(8.5,14,legend=c(levels(dat$Host.plant.species)[c(2,3,1,4)],"Bee leg length"), col = c(col_vector.short[unique(dat$Host.plant.species)], "darkorange"), pch=c(rep(19,4), 1),bty="n")
dev.off()
```
```{r spurvariation_neliana, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Spur length of plant populations that are associated with eight different populations of \\textit{R. neliana} bees. The bees used four different plant species across populations, \\textit{Diascia capsularis}, \\textit{Diascia fetcaniensis}, \\textit{Diascia rigescens} and \\textit{Diascia stachyoides}. The foreleg length of \\textit{R. neliana} in the different populations are shown in orange. Solid and open circles show the population mean of the plant spur length and bee leg length, respectively. The vertical lines extend one standard deviation above and below the mean."), echo=F}
knitr::include_graphics("spurvariation_neliana.pdf")
```

Figure \ref{fig:spurvariation_neliana} suggests that spur length varied relatively little across populations of *D. rigescens* while one of the *D. capsularis* populations had clearly shorter spurs. Most of the variation in spur length across populations was however due to bees visiting different plant species. Across populations, bee leg length appeared to be less variable than spur length
...
The only population where the variance in bee leg length was larger than the variance in spur length was population K but in this case, the bees were using a *Diascia* species with spurs that were shorter than the bees' legs. These data do not lend support to the idea that legs are under stabilising selection to fit a narrow range of spur lengths.
