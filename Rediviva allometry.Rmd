...
The analysis above does not account for non-independence among species due to phylogeny. We next examine whether the parameters of the species-specific allometric relationship estimated earlier (Table \ref{tab:alloparms} are related to spur length while accounting for phylogeny.
...
knitr::kable(tab, digits = 3, caption = 'Relationship between the allometric intercept and spur length of the host flower for 18 species of \\textit{Rediviva} bees. Beta is the estimated slope of this relationship and the other columns give the lower and upper 95% confidence limits.')
```
...
knitr::kable(tab, digits = 3, caption = 'Relationship between the allometric slope and spur length of the host flower for 18 species of \\textit{Rediviva} bees. Beta is the estimated slope of this relationship and the other columns give the lower and upper 95% confidence limits.')
```
...
```{r allometry_spurlength, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Relationship of allometric parameters (intercept a in the left-hand panel and slope b in the right-hand panel; see Equation \\ref{eq:allometry}) to spur length of the host plants for \\textit{Rediviva} bee species"), echo=F}
...
m11.n <- lm(loga ~ log(population_spur_length), weights=I(1/loga.se), data = allometries.n)
summary(m11.n)
```
...
```{r, warning=F}
m12.n <- lm(b ~ population_spur_length, weights=I(1/b.se), data = allometries.n)
summary(m12.n)
```
...
op <- par(mfrow=c(1,3))
plot(loga ~ log(population_spur_length), data=allometries.n, col=col_vector[population_code], axes=F, xlab="log(Spur length [mm])", ylab="log(Allometric intercept)", pch=19)
axis(1)
axis(2, las=1)
abline(m11.n)
...
```{r allometry_spurlength_neliana, out.width="\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Relationship of allometric parameters (intercept a in the left-hand panel and slope b in the right-hand panel; see Equation \\ref{eq:allometry}) to spur length of the host plants for \\textit{Rediviva neliana} populations"), echo=F}
